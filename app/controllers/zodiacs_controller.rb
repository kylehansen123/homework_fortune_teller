class ZodiacsController < ApplicationController
  ...
  def sign
    @name = params[:the_sign].downcase

    @prediction = Zodiac.find_by({ :sign => @name }).fortune

    render 'sign'

  end
  def creature
  @name = params[:the_creature].downcase

  @prediction = Zodiac.find_by({ :creature => @name }).fortune
